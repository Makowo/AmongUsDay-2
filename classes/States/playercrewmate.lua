PlayerCrewmate = PlayerCrewmate or class(PlayerStandard)

function PlayerCrewmate:init(unit)
	PlayerCrewmate.super.init(self, unit)
end

function PlayerCrewmate:enter(state_data, enter_data)
	PlayerCrewmate.super.enter(self, state_data, enter_data)
...
function PlayerCrewmate:_enter(enter_data)
	...
	--MenuCallbackHandler:hide_huds()
	managers.hud._hud_amongus:set_visible(true)
end

function PlayerCrewmate:exit(state_data, new_state_name)
	PlayerCrewmate.super.exit(self, state_data)
	MenuCallbackHandler:_update_outfit_information()
...
function PlayerCrewmate:update(t, dt)
	PlayerCrewmate.super.update(self, t, dt)
...
function PlayerCrewmate:_update_check_actions(t, dt)
...
function PlayerCrewmate:_check_action_interact(t, input)
...
function PlayerCrewmate:_start_action_interact(t, input, timer, interact_object)
	if _G.IS_VR then
...
function PlayerCrewmate:_interupt_action_interact(t, input, complete)
...
function PlayerCrewmate:_update_interaction_timers(t)
	if self._interact_expire_t then
...
function PlayerCrewmate:_check_action_jump(t, input)
	if input.btn_duck_press then
		--managers.hint:show_hint("clean_block_interact")
	end
end

function PlayerCrewmate:_check_action_duck(t, input)
	if input.btn_jump_press then
		--managers.hint:show_hint("clean_block_interact")
	end
end

function PlayerCrewmate:_check_action_run(t, input)
	if input.btn_run_press then
		--managers.hint:show_hint("clean_block_interact")
	end
end

function PlayerCrewmate:_check_action_use_item(t, input)
	if input.btn_use_item_press then
		--managers.hint:show_hint("clean_block_interact")
	end
end

function PlayerCrewmate:clbk_enemy_weapons_hot()
	if self._enemy_weapons_hot_listen_id then
		managers.groupai:state():remove_listener(self._enemy_weapons_hot_listen_id)
...
function PlayerCrewmate:interaction_blocked()
	return false
end

function PlayerCrewmate:_get_walk_headbob()
	return 0.0125
end
